...
def get_configured_logger(name):
    ...
    if (len(logger.handlers) == 0):
        ...
        if request.env.web2py_runtime_gae:
            ...
            formatter="%(asctime)s %(levelname)s:: %(name)s/%(module)s/%(funcName)s() | line %(lineno)d: %(message)s"
            handler = logging.handlers.RotatingFileHandler(os.path.join(request.folder,'{0}.log'.format(request.application)),maxBytes=104857600,backupCount=0)
